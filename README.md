...
## 3) Seed pgRouting (optional, for walking distances)
If you want `walk` values, import OSM into pgRouting. Otherwise, journeys will have `walk = 0` or be omitted.

```powershell
# Copy labeled.osm into the DB container
docker cp ..\labeled.osm transport-db:/tmp/labeled.osm

# Inside DB container (PowerShell runs bash):
docker exec -it transport-db bash -c "\
	osm2pgrouting -f /tmp/labeled.osm -d transport_db -U postgres -h localhost -W postgres \
		--clean --prefix ways --create --addnodes --attributes --tags"
```

Notes:
- Ensure `pgrouting` extension is installed (it is in the image).
- If `ways`/`ways_vertices_pgr` do not exist, `find_journeys_db` will skip the walk CTE gracefully.
